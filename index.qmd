...
- [FTIR Honey Analysis](#ftir-honey-analysis)
  ...
  - [Key Spectral Bands](#key-spectral-bands)
...
Honey is one of the most frequently adulterated foods globally. ATR-FTIR spectroscopy provides a rapid chemical fingerprint that can reveal differences in sugar composition, botanical origin, and the presence of adulterants such as syrup additions.

This notebook reads spectra directly from Perkin-Elmer Spectrum CSV exports, applies background correction, and performs PCA-based chemometric analysis on the carbohydrate fingerprint region (1800–650 cm⁻¹).

The notebook runs in **Google Colab** — upload your `.csv` files and run all cells. No installation required.

### What You Will Do

**Blank correction** — each sample spectrum is corrected by subtracting the nearest background (blank) spectrum, matched by instrument sample number.

**Spectral plots** — overlay and waterfall visualisations of the full range and fingerprint region, with diagnostic band annotations.

**PCA** — scores plot (PC1 vs PC2), loadings plot, and biplot identifying the wavenumber channels driving sample separation.

To use your own data, upload Perkin-Elmer Spectrum `.csv` files to `data/honey/`. Files starting with `blank` are automatically used as backgrounds; all other filenames become sample labels.

### Key Spectral Bands

| Wavenumber (cm⁻¹) | Assignment |
|-------------------|-----------|
| ~1640 | O-H bending (water) |
| ~1415 | C-H / O-H deformation |
| ~1155 | C-O-C stretch — sugar rings (sucrose diagnostic) |
| ~1080 | C-OH stretch — glucose |
| ~1025 | C-OH stretch — fructose |
| ~930  | Pyranose / furanose ring vibrations |

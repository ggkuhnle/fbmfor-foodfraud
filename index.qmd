...
Starch is a common adulterant in spices, cocoa, and dairy powders. Because different botanical sources produce starches with distinct crystal structures (polymorphs), X-ray diffraction (XRD) provides a rapid fingerprint that can reveal the botanical origin of a starch sample — and detect substitution.
...
1. **Waterfall plot** — visualise all five diffractograms offset for comparison
2. **Peak detection** — automated identification of Bragg peaks and assignment to polymorph type (A-type or B-type)
3. **Relative crystallinity** — estimate the ordered fraction using the method of Nara & Komiya
4. **Unknown identification** — match the Unknown peaks against the reference library and determine its botanical origin
...
- Wiklund, S. et al. (2008). Visualization of GC/TOF-MS-based metabolomics data for OPLS class models. *Anal. Chem.*, 80(1), 115–122.
